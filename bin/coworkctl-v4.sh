#!/bin/bash
# coworkctl v4 — move a Cowork workload between Al's Macs (Madoka <-> Ji-su).
#
# TRANSPORT: iCloud Drive only. Deliberately.
#
...
refresh(){
  ls "$HANDOFF" >/dev/null 2>&1; ls "$EVENTS" >/dev/null 2>&1
  [ -d "$EVENTS" ] && for d in "$EVENTS"/*/; do ls "$d" >/dev/null 2>&1; done
  ls "$PROJECTS" >/dev/null 2>&1
  sleep "${1:-2}"
}
...
emit(){
  mkdir -p "$EVENTS/$1"
  python3 - "$EVENTS/$1" "$1" "$2" "$ME" "$3" "$4" "$5" <<'PY'
...
cmd_status(){
  refresh
  echo "machine: $ME"
  echo "transport: iCloud (not synchronous — expect ~1min, longer if the peer slept)"
  echo
  local any=0
  if [ -d "$EVENTS" ]; then
    for d in "$EVENTS"/*/; do
...
cmd_handoff(){
  local project="${1:-}" to="" note="" wait_ack=0
  shift || true
  while [ $# -gt 0 ]; do
    case "$1" in
      --to) to="$2"; shift 2;;
  ...
  local doc; doc="$(cd "$PROJECTS/$project" && ls HANDOFF*.md 2>/dev/null | head -1)"
  [ -n "$doc" ] || echo "  note: no HANDOFF*.md — files will travel without context"
  echo "event: $(emit "$project" handoff "$to" "$note" "$doc")"
  echo
  echo "STAGED — not delivered. iCloud carries this in ~1min at best,"
...
cmd_log(){
  ...
  for f in "$EVENTS/$project"/*.json; do
    [ -f "$f" ] || continue
